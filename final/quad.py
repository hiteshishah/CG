...
class quad (simpleShape):
    ...
    def makeQuad (self) :
        ...
        for i in range(0, self.quadElementsLength - 2 ,3):
            ...
            self.addTriangleWithUV( self.quadVertices[point1 + 0],self.quadVertices[point1 + 1],
                                    self.quadVertices[point1 + 2], self.quadVertices[point2 + 0],
                                    self.quadVertices[point2 + 1], self.quadVertices[point2 + 2],
                                    self.quadVertices[point3 + 0],self.quadVertices[point3 + 1],
                                    self.quadVertices[point3 + 2], self.quadUV[normal1 + 0],
                                    self.quadUV[normal1 + 1],self.quadUV[normal2 + 0],self.quadUV[normal2 + 1],
                                    self.quadUV[normal3 + 0], self.quadUV[normal3 + 1])
