...
void main()
{
    ...
    else
    {
        ...
        // ambient
        vec4 ambient = texture2D(backTexture, texCoord) * ambient_reflection_coefficient * ambient_light;

        // diffuse
        vec4 diffuse = texture2D(backTexture, texCoord) * diffuse_reflection_coefficient * (dot(L, N));

        // specular
        vec4 specular = texture2D(backTexture, texCoord)  * specular_reflection_coefficient * pow(max(0.0, dot(R,V)), specular_exponent);
